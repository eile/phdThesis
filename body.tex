...
\cite{EEP:11} provides experimental results for a six-monitor tiled display
wall with up to 12 source GPUs. \fig{rCSLBFPS} provides an overview for the
achievable performance improvements. The first two configurations use a static
assignment of two GPUs to one output channel, where the first one statically
assigns one half to each GPU, and the second uses a sort-first load balancer to
dynamically split the segment. Already this 2D load-balancing improves the
...
across multiple segments. While theoretically the all-to-all ($12:6\times 12$)
configuration should provide the best performance, mispredictions of the
equalizers lead to a sweet spot of GPU sharing between segments. In our $12:6$
setup, assigning up to six GPUs per segment almost doubles the performance over
the state-of-the-art sort-first load balanced setup.

\fig{rCSLB} shows the rendering time over a fixed camera path rendering 540
frames. For the static assignment, two GPU were responsible for each of the six
outputs of the tiled display wall used. For the CSLB graph, all GPUs were
dynamically reassigned each frame to the six output channels, depending on the
current load distribution. Except for the few camera positions where the model
is distributed evenly over all outputs, CSLB easily outperforms the fixed
assignment.
