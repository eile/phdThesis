...
\begin{wrapfigure}{O}{.618\textwidth}
 \includegraphics[width=.618\textwidth]{results/cr}
 {\caption{\label{rCR}Performance of driving a Tiled Display Wall}}
\end{wrapfigure}

Compared to interception approaches such as used by Chromium, our asynchronous
programming model can inherintly provide better performance. \fig{rCR} tests
...
shown in \fig{rNuma}. This benchmark uses the aforementioned cluster nodes, and
...
\begin{wrapfigure}{O}{.382\textwidth}
...
Compositing is collecting and combining partial results from multiple resources
during scalable rendering onto one or more destination channel. While
...
is largely a generic problem and can be optimised by a parallel rendering
framework. Consequently, this area of parallel rendering research has received
significant attention. By integrating many state-of-the art optimizations into
our parallel rendering framework we provide a generic solution which scales
well on modern visualization cluster architectures. Furthermore, we present new
optimizations for NUMA-aware thread scheduling, insight into spatial sort-last
polygonal rendering, and image compression algorithms.
...
shown in \fig{fDepth} since these applications often do not sort their geometry
...
spatial sorting. Normally used for volume rendering, we have applied spatial
sort-last rendering and compositing to polygonal data, by sorting the data
spatially and applying clipping planes to generate perfectly convex rendering
subsets.

\begin{wrapfigure}{O}{.618\textwidth}
 \includegraphics[width=.618\textwidth]{results/dbSpatial}
 {\caption{\label{rDBSpatial}Spatial versus Depth-Sorted Sort-Last Rendering Performance}}
\end{wrapfigure}

\fig{rDBSpatial} shows the difference between spatial and depth-sorted
...
Direct send, shown in \fig{fDirectSend}, uses one operation on each resource to
fully composite a single tile. Binary swap, shown in \fig{fBS}, exchanges
...
\begin{wrapfigure}{O}{.618\textwidth}
 ...
 {\caption{\label{rROISF}Region of Interest for Sort-First Rendering Performance}}
\end{wrapfigure}

Applying ROI for sort-first rendering provides a small improvement for the
rendering performance, as shown in \fig{rROISF} from \cite{EBAHMP:12}. As the
...
\begin{wrapfigure}{O}{.618\textwidth}
 ...
 {\caption{\label{rROISL}Region of Interest for Sort-Last Rendering Performance}}
\end{wrapfigure}
...
\begin{wrapfigure}{O}{.618\textwidth}
 ...
 {\caption{\label{rAsync}Asynchronous Compositing Sort-First Rendering Performance}}
\end{wrapfigure}
...
\fig{rAsync} from \cite{EBAHMP:12}, pipelining the readback with the rendering
...
\begin{wrapfigure}{O}{.618\textwidth}
  ...
  \caption{Image compression performance in Sort-First Polygonal Rendering}
  \label{rRLE}
\end{wrapfigure}

\fig{rRLE} summarizes the compression results from \cite{MEP:10}. We have
...
resource load. Static load balancing, shown in \fig{floadbalancing} top,
performs this assignment during initialization. Dynamic load balancing can
either be reactive or predictive (middle and bottom of \fig{floadbalancing}).
...
\begin{wrapfigure}{I}{.382\textwidth}
...
\fig{rCompounds} shows the performance of static and load-balanced sort-first
...
\fig{rEqualizers} provides a more detailed analysis of the different
...
\fig{rEqualizers} (left) gives an impression of how individual modes perform on
...
compute resources. \fig{rEqualizers} (right), on the other hand, illustrates
...
\begin{wrapfigure}{O}{.618\textwidth}
  ...
  \caption{\label{rCSLBFPS}Cross-Segment Load Balancing Performance}
\end{wrapfigure}

\cite{EEP:11} provide experimental results for a six-monitor tiled display wall
with up to 12 source GPUs. \fig{rCSLBFPS} shows an overview for the achievable
...
half to each GPU, and the second uses a sort-first load balancer to dynamically
split the segment between the two GPUs. Already this 2D load-balancing improves
the framerate by almost 50\%.
...
\fig{rCSLB} shows the rendering time over a fixed camera path rendering 540
frames. For the static assignment, two GPUs are responsible for each of the six
outputs of the tiled display wall used. For the CSLB graph, up to eight GPUs
were dynamically reassigned each frame to each of the six output channels,
depending on the current load distribution. Except for the few camera positions
where the model is distributed evenly over all outputs, CSLB easily outperforms
...
\includegraphics[height=.245\textwidth]{images/beechnut}\vspace{1em}\\
  ...
  \caption{\label{rMapping}Object Mapping and Synchronization Performance}
\end{figure}

\fig{rMapping} analyses the performance how data distribution and
...
consistent with the benchmarks in \fig{rCompressorDetail}; RLE, Snappy and the
...
\fig{rCompress} shows the compression ratio and speed for generic binary data from \cite{ESP:18}. While the structure of the transmitted data varies with each application, this micro-bench\-mark gives a reasonable estimation of the expected performance. In our context of interactive distributed rendering applications, it is important to use the right tradeoff between spending time and resources for data compression, and the gained network transmission time due data reduction.
...
\begin{figure}[h!t]\center
  ...
  \caption{\label{rCompressorDetail}Data compression for the PLY data and raw volumes shown in \fig{rMapping}}
\end{figure}

In \cite{ESP:18}, we also evaluated the compression performance for more
concrete application data. The results are shown in \fig{rCompressorDetail}.
...
\fig{rNetwork} compares data distribution speed using different network
