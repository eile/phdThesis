...
In the following we will first describe the execution model and resource
...
In practical scenerios, application initialisation and exit is also a factor
for usability. Consequently, these phases are also parallelised in Equalizer. A
first pass identifies the resources to be launched or terminated, kicks of the
...
This benchmarks simply measures the time taken by \textsf{Config::init}, which
...
and window initialisation, and object data mapping for the Equalizer resource
...
a set of C++ classes, modelled closely to the resource hierarchy of a
...
the cores of the processor close to the respective GPU.
...
Compounds only provide a static configuration of the parallel rendering setup.
\textsf{Equalizers} are algorithms which hook into a compound and modify
...
\textsf{Equalizer} and other applications. A barrier is a distributed primitive
...
Queues are distributed, single producer, multiple consumer FIFO queues used for
...
research quesiton on what new algorithms will decrease the time needed to
...
with an algorithm which automatically computes the ROI by analyzing the
...
that can serve both complex applications and research, has been no easy task. (due to conflicting requirements?)
...
enables the easy? implementation of new decomposition algorithms, many
improvements for result composition, novel load balancing schemes, and numerous
whole system optimisations, all of which are much harder(quanitfy, life is hard) without such a
...
provides a novel(repeat) approach to better/improve assign/ing multiple rendering resources to
...
parallel rendering system. (is mehr als nur ne cherry, oder?)

Beyond the scope of this thesis, Equalizer has influenced the field and has
been used in various commercial and research applications (link/quote). These applications
...
with exception of keeping up with new hardware technologies, e.g. providing glue
code for the Vulkan API, or exploiting new Multi-GPU extensions. A
large amount of work remains to make parallel rendering more accessible (to?). This may be
addressed by simplified APIs layered on top of Equalizer, and through integrations with
...
There is still a significant amount of research in automatically selecting the best
decomposition and recomposition algorithm as well as the resources used
for a given application. This task becomes even more challenging when
considering changes/fluctuations? of the rendering load and algorithm during the runtime of
