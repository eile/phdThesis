...
to gain intuitive insight into spatial structures and the relations of very large
...
fidelity. The quadratic increase in pixels to be rendered,
increases the pressure on rendering algorithms to deliver an interactive
frame rate (hz?). Such large-scale visualization systems are often driven by multiple
...
are also affordable for on-site hosting for many organisations. They have become
the <bla> of choice.
...
existing software is often engineered for different design parameters (being?) and
...
Reality (VR) takes this goal further, and when done right, makes the user
...
Sort-last rendering decomposes the rendering task at the data level, so
...
hardware and scalability of clusters. (+cost?)
...
If the OpenGL stream is large in size, due to not only containing OpenGL
...
\begin{compactitem}
  ...
  \sref{sAsyncExec} introduces our asynchronous execution model, which has been carefully designed to minimise synchronisation points, maximise pipelining and enables early display of rendered images.\\[\medskipamount]
...
dominant factor in most polygonal rendering codes. Since this decomposition can
...
each GPU, depending on the current camera position. For volume rendering, this
balance is reversed. Typically, a volume renderer is bound in fragment
processing. COnsequently, sort-first rendering scales better than sort-last,
since the screenspace is equally divided. For both renderer, one can nicely
observe static imbalances in the zig-zag graphs, where odd number of resources
coincidentally split the rendering load less balanced than even numbers.
...
Pixel compounds
provide predictable scalability, but fail to approach ideal linear scaling due
to their increased compositing cost and constant geometry load.
