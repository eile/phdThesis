...
fueled by the pervasive diffusion of digitalisation in modern life. Moreover,
...
\begin{figure}[h!t]
 ...
 \includegraphics[height=4.95cm]{images/tide.jpg}%
...
fidelity, but with the quadratic increase in pixels to be rendered, it
increases the pressure on rendering algorithms to deliver an interactive
frame rate. Such large-scale visualization systems are often driven by multiple
GPUs and workstations, making it natural, and most times necessary, to drive
them using parallel and distributed applications.
...
process turns interactive visualization into a powerful tool, since it utilises
...
Parallel rendering utilises multiple rendering units (GPUs), potentially on
...
each resource renders a part of the data, and in the end partial fragments from
each resource are composited to form a final results. Sort-middle rendering
...
In this chapter we summarise the main contributions of this thesis. For each
section, we list the relevant publications and specify the contributions of the
author.

\section{Parallel Rendering Architecture}

A major contribution of this thesis is the formalisation and implementation of
...
to load balance the tasks executed in parallel, to minimise synchronisation and
...
rendering parameters, as shown in \fig{FIG_execution}. Similarly, the
...
programming model can inherently provide better performance. \bench{rCR} tests
...
setup is favourable for Chromium because the display lists have to be
...
The application requests a new frame to be rendered, synchronises on the
completion of a frame and processes events received from the render clients. It
may perform idle processing between the start and synchronisation of a frame.
\fig{FIG_execution} shows a simplified execution model of an Equalizer
application.
...
semantics for thread safe event handling. For Qt, which centralises the event
...
\fig{fNumaNode} shows one such NUMA visualization node, used in the experiments
...
\fig{fNumaThreads} shows the thread placement for the example node used in
\fig{fNumaNode}. The threads are placed to all cores of the respective
...
shown in \bench{rNuma}. This benchmark uses the aforementioned cluster nodes, and
...
small influence, it becomes more important with higher frame rates as the
...
vertical field-of-view and distance of the projection wall. \fig{fFrusta}
...
output, a canvas swap barrier or a swap barrier on each segment synchronise the
...
given in \fig{FIG_leaf_compound}. The data range is a logical mapping of the
...
\begin{wrapfloat}{figure}{O}{.618\textwidth}
  ...
  {\caption{\label{fTide}Tiled Display Wall showing remote rendering of the Equalizer-based RTNeuron Application}}
\end{wrapfloat}
...
arbitrary applications from remote sources to be shown. \fig{fTide} shows our
...
content to a single window on the wall, enabling parallel streaming for
parallel rendering applications. The stream library also implements an event
model, where an application can register itself to receive keyboard, mouse and
window management events from the display wall.

We integrated the stream library into Equalizer to send the framebuffer of each
destination channel to DisplayCluster, using a direct FBO download (if
possible) or a texture download. The compression and sending uses asynchronous
transmission, pipelining compression and streaming with rendering. Received
events from DisplayCluster are converted and forwarded to Equalizer’s event
system. This integration allows all Equalizer applications to benefit from
streaming without any code changes, it is configured simply by specifying the
DisplayCluster hostname on all views to be streamed.
...
different tile sizes ($256^2$, $512^2$, $1024^2$ and $1920\times 1080$). Due to a
configuration issue, the WAN link delivered only 1 GBit/s throughput during the
benchmark test. RTNeuron is an Equalizer-based application used in the Blue
...
native resolution, that a $512^2$ tile size is the best option found, and that
95\% compression delivers the best performance in most cases. Based on the
...
\begin{figure}[t]\center
  \subfigure[]{
 \includegraphics[width=.31\textwidth]{images/Pixel}
    \label{fPixel}
  }\hfil
  \subfigure[]{
    \includegraphics[width=.31\textwidth]{images/Subpixel}
    \label{fSubpixel}
  }\hfil
  \subfigure[]{
    \includegraphics[width=.31\textwidth]{images/tile}
    \label{fTiles}
  }
...
Since subpixel compounds increase the amount of pixels to be composited
...
Tile (\fig{fTiles}) decompositions are a variant of sort-first rendering. They
...
compositing is often used for polygonal data, as shown in \fig{fDepth} since
...
optimization (\sref{sEarlyAss}), direct send can handle imbalances between
...
rendering performance, as shown in \bench{rROISF} from \cite{EBAHMP:12}. As the
...
which automatically computes the ROI by analyzing the framebuffer to maximise
...
\bench{rAsync} from \cite{EBAHMP:12}, pipelining the readback with the rendering
yields a performance again of about 10\%. At higher frame rates, when the
...
The graph shows how the various incremental improvements add up to significant performance gains. Even in a relatively difficult scenario with a fast rendering time and, by modern standards, slow network interconnect, we were able to more than double the performance to above 60~Hz.

The basic 8-byte RLE Compressor performs just minimally better than no compression. Both stay relatively close the theoretical maximum, but can't quite reach it due to load imbalances and non-zero rendering time. The swizzling pre-conditioner can significantly reduce the compositing time, and even improve the overall framerate.
...
shown in \fig{floadbalancing} top, performs this assignment during
initialization. Dynamic load balancing can either be reactive or predictive
(middle and bottom of \fig{floadbalancing}). Reactive load balancing utilises
...
grid as shown \fig{fROI}, top. In cases where the rendered data projects only
...
Implicit load balancing generally uses a finer granularity of many more tasks
...
\bench{rCompounds} shows the performance of static and load balanced sort-first
...
\bench{rEqualizers} (left) gives an impression of how individual modes perform on
...
compute resources. \bench{rEqualizers} (right), on the other hand, illustrates
...
rendering nodes in virtualised environments. For this scenario tile equalizer
...
The \textsf{tile\_equalizer} often outperforms \textsf{tree\_equalizer}. This
...
\fig{fViewEqualizer} depicts a snapshot of a simple CSLB setup along with its
...
driven by 12 source GPUs. \bench{rCSLBFPS} shows an overview for the achievable
...
The monitor equalizer (\fig{fmonitor}) allows to reuse the rendering from one
...
Researching and improving the system behaviour of non-trivial applications is
...
\bench{rMapping} analyses the performance how data distribution and
synchronisation performs in real-world applications. We extracted the existing
...
noticeable measurement jitter due to other services running on the shared
...
unbuffered data, \textsf{buffered} reuses uncompressed, serialised data for
...
deserialise and decompress the data. During data synchronisation, the master
...
During commit, the buffers only have to be serialised and compressed once, and
can then be sent directly to all mapped slave nodes. Slave nodes queue this
data and consume it during synchronisation. In contrast, object mapping needs
to wait for each slave node to request the mapping, and then may need to
reserialise and compress the object data. We test the time to commit and sync
the data using different compression engines discussed under optimisations
below.
...
total of 1023 objects are distributed and synchronised. Due to limited
...
compression overhead cannot be amortised by the decreased transmission time.
...
although only by about 10\%. During synchronisation, data is pushed from the
...
performance when amortised over many send operations.
...
implement various optimizations for faster mapping and synchronisation of data:
...
\bench{rCompress} shows the compression ratio and speed for generic binary data from \cite{ESP:18}. While the structure of the transmitted data varies with each application, this micro-bench\-mark gives a reasonable estimation of the expected performance. In our context of interactive distributed rendering applications, it is important to use the right tradeoff between spending time and resources for data compression, and the gained network transmission time due data reduction.
...
\begin{benchmark}[h!t]\center
  ...
  \caption{\label{rCompressorDetail}Data Compression for the PLY Data and Raw Volumes from \bench{rMapping}}
\end{benchmark}

In \cite{ESP:18}, we also evaluated the compression performance for more
concrete application data. The results are shown in \bench{rCompressorDetail}.
...
significant contributions in this area. The base framework allowed us to take
parallel rendering research to a new level, by enabling the easy implementation
of new decomposition algorithms, many improvements for result composition,
novel load balancing schemes, and numerous whole system optimizations, all of
which are much harder without such a framework and associated applications.
This is not only supported by the contributions of this thesis, but by other
publications and doctoral theses completed using Equalizer. This research has
not only been performed in the original research group; Equalizer has also been
picked up by other laboratories, e.g., the Electronic Visualization Laboratory
at the University of Illinois at Chicago for Cave2 research.
...
task to distribute and synchronise application state in a parallel rendering
system.
...
code for the Vulkan API. There remains a large amount of work to make parallel
rendering more accessible. This may be addressed by simplified APIs on top of
Equalizer, and through integrations with popular rendering toolkits. On the
other hand, it should address operators and users of visualization systems
through simplified configuration, monitoring and administration tools. In the
same spirit, there is still a significant amount of research in automatically
selecting the decomposition and recomposition algorithm as well as the
resources to be used for a given application. This task becomes even more
challenging when considering changes of the rendering load and algorithm during
