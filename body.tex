...
The {\em pipe} is the abstraction of a graphic card (GPU), and uses an
...
The {\em channel} is the abstraction of an OpenGL viewport within its parent
...
specify {\em OFF} as a layout, which deactivates the canvas. It is possible
...
{\em Equalizer} does support multiple, independent tracked views through the
...
{\em Equalizers} are algorithms which hook into a compound and modify
...
\label{sec:Serializable}The {\em Collage} Serializable implements one
convenient usage pattern for object data distribution which emerged during
deployment of Equalizer in applications. The {\em co::Serializable} data
