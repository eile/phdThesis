...
increases the pressure on rendering algorithms to deliver interactive
framerates. Such large-scale visualization systems are often driven by multiple
...
unshaded primitives before or after rasterization, and then performs the
...
programming model can inherintly provide better performance. \bench{rCR} tests
...
extent, Omegalib, implement a multi-window enviroment around a shared
...
compressed using libTurboJPEG, and sent asynchronously and in parallel by the
...
The results show that interactive framerates are available even at the full
...
avoid artifacts as the default settings in Equalizer.
...
optimize the resource usage. Each equalizer focuses on tuning one aspect of
...
\bench{rRLE} summarizes the compression results from \cite{MEP:10}. We have
...
(middle and bottom of \fig{floadbalancing}). Reactive load balancing utilizes
...
\includegraphics[height=.245\textwidth]{images/beechnut}\vspace{1em}\\
  ...
  \caption{\label{rMapping}Object Mapping and Synchronization}
\end{benchmark}
